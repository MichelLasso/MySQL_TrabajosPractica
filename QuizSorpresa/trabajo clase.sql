...
-- funciones 
-- 1. obtener el total de pedidos realizado por un cliente
delimiter //
create function pedidosTotal(idCliente int)
returns int deterministic
begin 
	declare total int;
    set total= (select count(id_cliente) from pedido where idCliente=id_cliente);
	return total;
end //
delimiter ;

select pedidosTotal(6);

-- 2. calcular la comisión total ganada por un comercial
delimiter //
create function comisionTotal(idComersial int)
returns float deterministic
begin 
	declare totalComision float;
    set totalComision= (select sum(comision) from comercial
						inner join pedido on comercial.id=pedido.id_comercial
                        where idComersial=comercial.id
                        );
	return totalComision;
end // delimiter ;
drop function comisionTotal;
select comisionTotal(1);

-- 3. obtener el cliente con mayor total de pedidos
delimiter //
create function clienteMasPedido()
returns int deterministic
begin 
	declare total int;
    set total= (select count(id_cliente) from pedido group by id_cliente order by 1 desc
				limit 1);
	return total;
    
end // delimiter ;
drop function clienteMasPedido;
select clienteMasPedido();

-- 4. Contar la cantidad de pedidos realizados en un año especifico
delimiter //
create function pedidosTotalAño(año varchar(30))
returns int deterministic
begin 
	declare total int;
    set total= (select count(id_cliente) from pedido where año=fecha);
	return total;
end //
delimiter ;
select pedidosTotalAño('2024-02-15');

-- 5. Obtener el promedio de total de pedidos por cliente
delimiter //
create function promedioPedidoCliente(idCliente int)
returns int deterministic
begin 
	declare total int;
    set total= (select count(id_cliente) from pedido where idCliente=id_cliente);
...
delimiter ;
select avg(promedioPedidoCliente(6));